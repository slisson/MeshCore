...
void CommonCLI::loadPrefsInt(FILESYSTEM* fs, const char* filename) {
  ...
  if (file) {
    ...
    file.read((uint8_t *)&_prefs->qos_packets_per_hour, sizeof(_prefs->qos_packets_per_hour)); // 290
    file.read((uint8_t *)&_prefs->rx_boosted_gain, sizeof(_prefs->rx_boosted_gain));              // 292
    file.read((uint8_t *)&_prefs->flood_max_unscoped, sizeof(_prefs->flood_max_unscoped));   // 293
    file.read((uint8_t *)&_prefs->flood_max_advert, sizeof(_prefs->flood_max_advert));       // 294
    // next: 295
...
void CommonCLI::savePrefs(FILESYSTEM* fs) {
  ...
  if (file) {
    ...
    file.write((uint8_t *)&_prefs->qos_packets_per_hour, sizeof(_prefs->qos_packets_per_hour));  // 290
    file.write((uint8_t *)&_prefs->rx_boosted_gain, sizeof(_prefs->rx_boosted_gain));              // 292
    file.write((uint8_t *)&_prefs->flood_max_unscoped, sizeof(_prefs->flood_max_unscoped));   // 293
    file.write((uint8_t *)&_prefs->flood_max_advert, sizeof(_prefs->flood_max_advert));       // 294
    // next: 295
...
void CommonCLI::handleGetCmd(uint32_t sender_timestamp, char* command, char* reply) {
  ...
  if (memcmp(config, "dutycycle", 9) == 0) {
    ...
    sprintf(reply, "> %d", (uint32_t)_prefs->qos_packets_per_hour);
